...
(async function main() {
  ...
  if (firstCommitMessage) {
    ...
    if (!githubIssue) {
      ...
      if (!githubIssue.labels.every((l) => l.includes(githubLabel))) {
  ...
  if (hasChangelog) {
    ...
    if (!diff) {
      ...
      const changelogEntry = parseChangelogEntry(diff.after);
      if (!changelogEntry) {
        fail(`CHANGELOG entry does not match format.`);
      } else {
        if (firstCommitMessage.issueId !== changelogEntry.issueId) {
          fail(
            `First commit message issue id differs from CHANGELOG entry issueId: "${firstCommitMessage.issueId}" vs. "${changelogEntry.issueId}".`
          );
        }

        if (changelogEntry.issueId !== changelogEntry.linkIssueId) {
          fail(
            `CHANGELOG entry issue id differs from CHANGELOG entry link issue id: "${changelogEntry.issueId}" vs. "${changelogEntry.linkIssueId}".`
          );
        }
  ...
  function parseChangelogEntry(changelog: string) {
    const changelogRegex =
      /^\* (?<summary>[^\s].+[^\s]) \(\[#(?<issueId>\d+)\]\(https:\/\/github\.com\/dso-toolkit\/dso-toolkit\/issues\/(?<linkIssueId>\d+)\)\)$/m;
    ...
    const { summary } = match.groups;
    const { index } = match;

    if (isNaN(issueId) || isNaN(linkIssueId) || typeof summary !== "string" || typeof index !== "number") {
      return null;
    }
