...
examplePageFactory(
  'Patronen',
  'Beheer Basis',
  ({ headerTemplate, breadcrumbsTemplate, buttonTemplate, paginationTemplate, definitionListTemplate, tableTemplate }, templates) => {
    const rowActions = () => html`${buttonTemplate({ type: 'button', variant: 'tertiary', label: 'Bewerk', icon: { icon: 'pencil' }, iconMode: 'only' })} ${buttonTemplate({ type: 'button', variant: 'tertiary', label: 'Verwijder', icon: { icon: 'trash' }, iconMode: 'only' })}`
