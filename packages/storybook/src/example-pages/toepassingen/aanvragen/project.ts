  ...
  ({ applicationHeadingTemplate, alertTemplate, formTemplate, formButtonsTemplate }) => html`
    <div class="container">
      <main>
        ...
        ${alertTemplate({ status: AlertType.Info, message: html`Hulp nodig bij kiezen? Doe dan eerst de <a href="#">Vergunningscheck</a>.` })}
        ${formTemplate(form)}
        ${formButtonsTemplate({ buttons: [{ label: 'Volgende stap', variant: 'primary', type: 'submit', icon: { icon: 'chevron-right' }, iconMode: 'after' }] })}
      </main>
    </div>
  `
);
