...
export const cssFormGroupRadios: ComponentImplementation<FormGroupRadios<TemplateResult>> = {
  ...
  implementation: 'css',
  template: ({ infoButtonTemplate, infoTemplate, selectableTemplate }) => function formGroupRadiosTemplate(formGroup) {
    ...
    return html`
      <fieldset
        class="form-group dso-radios ${classMap({ 'dso-required': !!formGroup.required, 'dso-inline': !!formGroup.inline, [`dso-${formGroup.state}`]: !!formGroup.state })}"
