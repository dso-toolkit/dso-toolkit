...
export const cssModal: ComponentImplementation<Modal<TemplateResult>> = {
  ...
  template: ({ buttonTemplate }) =>
    function modalTemplate({ body, buttons, confirm, role, heading, id }) {
      return html`
        <div
          tabindex="-1"
          ...
          <div class="dso-dialog" role="document">
            ${heading && id
              ? html`
                ...
                `
              : nothing}
            <div class="dso-body">${typeof body === "string" ? unsafeHTML(body) : body}</div>
            ${buttons?.length
              ? html` <div class="dso-footer">${buttons.map((b) => buttonTemplate(b))}</div> `
              : nothing}
          </div>
        </div>
      `;
