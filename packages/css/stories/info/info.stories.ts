...
const infoTemplate = <TemplateResult>({ fixed, richContent, onClose }: Info<TemplateResult>) => html`
  <div class="dso-info">
    ${fixed
      ? html`
...
storiesOfInfo<TemplateResult>({
  ...
  storiesOf,
  readme,
  template: ({ fixed, richContent, close }: InfoArgs<TemplateResult>) => infoTemplate<TemplateResult>({ fixed, richContent, onClose: close })
}, {
  richContent
});
