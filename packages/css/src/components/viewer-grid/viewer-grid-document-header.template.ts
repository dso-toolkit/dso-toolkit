...
export function viewerGridDocumentHeaderTemplate({ title, type, features, featuresAction, mapAction, featuresOpen, owner, status }: ViewerGridDocumentHeader<TemplateResult>) {
  return html`
    <div class="dso-document-header">
      ${title}

      <div class="dso-document-header-container">
        <p class="dso-document-header-type">${type}</p><p class="dso-document-header-owner">${owner}</p>
