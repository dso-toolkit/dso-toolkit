...
function watcher() {
  ...
  gulp.watch(`*.(svg|scss)`, { cwd: iconsPath }).on("all", function (event, _path, _stats) {
    log("icons", event);
...
function copyAssets() {
  ...
  return gulp.src("**", { cwd: assetsPath }).pipe(gulp.dest(assetsDistPath));
}
