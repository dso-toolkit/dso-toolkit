describe('Date Picker', () => {
  ...
  it('should emit keyboard events', () => {
    ...
    cy.get('dso-date-picker').then(datePicker => {
      datePicker.get(0).addEventListener('dsoKeyDown', (event: CustomEvent) => keys.push(event.detail.event.key))
      datePicker.get(0).addEventListener('dsoKeyUp', (event: CustomEvent) => keys.push(event.detail.event.key))
    });
