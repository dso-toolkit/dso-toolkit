...
function getColspan({ columns }: Colspecs, nameStart: string | null, nameEnd: string | null): number | undefined {
...
export const Cell: FunctionalComponent<{ context: OzonContentNodeContext, colspecs: Colspecs | undefined, cell: Element }> = ({ context: { mapNodeToJsx }, colspecs, cell }) => {
  const {
    moreRows,
    nameStart,
    nameEnd
  ...
  const td: JSXBase.TdHTMLAttributes<HTMLTableCellElement> = {
    ...
    colSpan: colspecs ? getColspan(colspecs, nameStart, nameEnd) : undefined
  };

  return (
    <td {...td}>{mapNodeToJsx(cell.childNodes)}</td>
  );
};
