...
export function storiesOfWhitebox<Implementation, Templates, TemplateFnReturnType>(storiesOfArguments: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, WhiteboxTemplates<TemplateFnReturnType>>) {
  return storiesOfFactory('Whitebox', storiesOfArguments, (stories, templateMapper) => {
    ...
    const template = templateMapper<WhiteboxArgs>((args, { whiteboxTemplate }) => whiteboxTemplate(whiteboxArgsMapper(args)));
