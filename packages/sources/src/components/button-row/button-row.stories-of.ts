...
export function storiesOfButtonRow<Implementation, Templates, TemplateFnReturnType>(storiesOfArguments: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, ButtonRowTemplates<TemplateFnReturnType>>) {
  return storiesOfFactory('Button Row', storiesOfArguments, (stories, templateMapper) => {
    ...
    const template = templateMapper<ButtonRowArgs>((args, { buttonRowTemplate }) => buttonRowTemplate(buttonRowArgsMapper(args)));
