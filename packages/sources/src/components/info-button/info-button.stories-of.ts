...
export function storiesOfInfoButton<Implementation, Templates, TemplateFnReturnType>(storiesOfArguments: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, InfoButtonTemplates<TemplateFnReturnType>>) {
  return storiesOfFactory('Info Button', storiesOfArguments, (stories, templateMapper) => {
    ...
    const template = templateMapper<InfoButtonArgs>((args, { infoButtonTemplate }) => infoButtonTemplate(infoButtonArgsMapper(args)));
