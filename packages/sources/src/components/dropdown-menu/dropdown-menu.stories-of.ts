...
export function storiesOfDropdownMenu<Implementation, Templates, TemplateFnReturnType>(storiesOfArguments: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, DropdownMenuTemplates<TemplateFnReturnType>>) {
  return storiesOfFactory('Dropdown Menu', storiesOfArguments, (stories, templateMapper) => {
    ...
    const template = templateMapper<DropdownMenuArgs>((args, { dropdownMenuTemplate }) => dropdownMenuTemplate(dropdownMenuArgsMapper(args)));
