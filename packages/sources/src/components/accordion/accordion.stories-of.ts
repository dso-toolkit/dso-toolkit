...
export function storiesOfAccordion<Implementation, Templates, TemplateFnReturnType>(storiesOfArguments: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, AccordionTemplates<TemplateFnReturnType>>) {
  return storiesOfFactory('Accordion', storiesOfArguments, (stories, templateMapper) => {
    ...
    const template = templateMapper<AccordionArgs>((args, { accordionTemplate }) => accordionTemplate(accordionArgsMapper(args)));
