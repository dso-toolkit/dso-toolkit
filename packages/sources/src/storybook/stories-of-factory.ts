...
export interface StoryOfFactoryArguments<TemplateFnReturnType, StoryTemplates> {
  (
    ...
    templateMapper: <StoryVariantArgs>(cb: (args: StoryVariantArgs, storyTemplates: StoryTemplates) => TemplateFnReturnType) => (args: Args, context: any) => TemplateFnReturnType
  ): void;
}

export function storiesOfFactory<Implementation, TemplateFnReturnType, StoryTemplates, Templates>(
  name: string,
  { parameters, storyTemplates, templateContainer }: StoriesOfArguments<Implementation, Templates, TemplateFnReturnType, StoryTemplates>,
