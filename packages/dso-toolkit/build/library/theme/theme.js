(function () {
	...
	function isSmallScreen() {
	  return jquery(document).width() < config.breakpoints.navCollapse;
	}
	...
	var Tree = /*#__PURE__*/function () {
	  function Tree(el) {
	    ...
	    this._collections = jquery.map(this._el.find('[data-behaviour="collection"]'), function (c) {
	  ...
	  _createClass(Tree, [{
	    ...
	    value: function filter(value) {
	      var cleanRegex = /[ -]/g;
	      var items = jquery('*[data-role="item"]', this._el);
	      value = (value || '').toLowerCase().replace(cleanRegex, '');

	      if (value) {
	        items.each(function (i, e) {
	          var item = jquery(e);
	          var text = jquery('a, button', item).text().trim().toLowerCase().replace(cleanRegex, '');
	...
	var Filter = /*#__PURE__*/function () {
	  function Filter(filterInput, filteredItems) {
	    ...
	    this.clearButton = jquery("#".concat(this.filterInput.data("clear-button")));
	...
	var VersionSelectorService = function VersionSelectorService() {
	...
	_defineProperty(VersionSelectorService, "getVersions", function () {
	  ...
	  VersionSelectorService._versionsPromise = jquery.ajax('https://www.dso-toolkit.nl/versions.json?t=' + new Date().getTime()).then(function (results) {
	...
	var VersionSelector = /*#__PURE__*/function () {
	  ...
	  _createClass(VersionSelector, [{
	    ...
	    value: function _toggleSelector(e, hint) {
	      e.stopPropagation();
	      var navItem = jquery('#version_selector_nav_item');
	      var button = jquery('#version_selector_button');
	      var dropdownMenu = jquery('#version_selector_dropdown_menu');

	      if (button.attr('aria-expanded') === 'true' || hint === 'close') {
	        navItem.removeClass('open');
	        dropdownMenu.hide();
	        button.attr('aria-expanded', 'false');
	        jquery(window).off('.version-selector');
	      } else {
	        navItem.addClass('open');
	        dropdownMenu.show();
	        button.attr('aria-expanded', 'true');
	        jquery(window).on('click.version-selector', this._windowClicked.bind(this));
	        jquery(window).on('keydown.version-selector', this._windowKeyDown.bind(this));
	      }
	    }
	  }, {
	    key: "_windowClicked",
	    value: function _windowClicked(e) {
	      if (jquery(e.target).parents('.toolkit-version-selector').length === 1) {
	        return;
	      }
	    ...
	    value: function _render(versions) {
	      ...
	      var container = jquery('.toolkit-version-selector');
	      ...
	      jquery('#version_selector_releases').append(versions.releases.map(createListItem));
	      jquery('#version_selector_branches').append([{
	        version: 'master'
	      }].concat(versions.topicBranches).map(createListItem));
	      var button = jquery('#version_selector_button');
	      button.on('click', this._toggleSelector.bind(this)).trigger('click');
	      var buttonLabel = button.find('> span.label');
	...
	var frame$1 = frame(jquery('#frame'));
	var navTrees = jquery.map(jquery('[data-behaviour="tree"]'), function (t) {
	  return new Tree(t);
	});
	new Filter(jquery('#menu-filter'), navTrees);
	loadPen();
	new VersionSelector();
	...
	function loadPen() {
	  setTimeout(function () {
	    jquery.map(jquery('[data-behaviour="pen"]'), function (p) {
	      return new Pen(p);
	    });
	  }, 1);
	}
