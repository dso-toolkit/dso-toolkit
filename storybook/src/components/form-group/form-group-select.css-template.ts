import { FormGroupSelect, SelectOption } from "dso-toolkit";
    ...
    function formGroupSelectTemplate(formGroup) {
      ...
      return html`
        ${formGroup.animatable
          ? html`<dso-expandable open enable-animation>${renderFormGroupSelect()}</dso-expandable>`
          : renderFormGroupSelect()}
      `;

      function renderFormGroupSelect() {
        return html`
          <div
