...
describe("Tree View", () => {
  function firstChildItem(subject: JQuery<HTMLElement>, label: string) {
    return cy.wrap(subject).parent().find("> ul > li > p.tree-content").first().should("have.text", label).as(label);
  }

  function lastChildItem(subject: JQuery<HTMLElement>, label: string) {
    return cy.wrap(subject).parent().find("> ul > li > p.tree-content").last().should("have.text", label).as(label);
  }

  function nextSiblingItem(subject: JQuery<HTMLElement>, label: string) {
    return cy.wrap(subject).parent().next().find("> p.tree-content").first().should("have.text", label).as(label);
  }

  function previousSiblingItem(subject: JQuery<HTMLElement>, label: string) {
    return cy.wrap(subject).parent().prev().find("> p.tree-content").first().should("have.text", label).as(label);
  }

  function shouldHaveFocusAndTabIndex(subject: JQuery<HTMLElement>) {
    return cy.wrap(subject).should("have.focus").should("have.attr", "tabIndex", 0);
  }
  ...
  it("should have correct aria attributes for 'tree item sets'", () => {
    function shouldHaveCorrectAriaTreeItemAttributes(
      startElement: string,
      level: number,
      setSize: number,
      posInSet: number,
  ...
  it("should expand / collapse on click on the controller icon", () => {
    cy.get("@first-tree-item").prev().click();
  ...
  it("should focus next item starting with search letter", () => {
    cy.get("@first-tree-item").then(shouldHaveFocusAndTabIndex).realPress("b");

    cy.get("@first-tree-item").then(shouldNotHaveFocusAndTabIndex);
  ...
  it("should expand or focus child on right arrow key and collapse or focus parent left arrow key", () => {
    function shouldHaveExpandedBehaviour(
      ...
      return cursor.then((subject) => (keys ? press(subject, keys) : subject));
    }

    cy.get("@first-tree-item").then(shouldHaveFocusAndTabIndex).realPress(["ArrowDown"]);

    cy.get("@first-tree-item").then((subject) => firstChildItem(subject, "bouwelementen"));

    shouldHaveExpandedBehaviour("bouwelementen", false, ["ArrowRight"]);
    shouldHaveExpandedBehaviour("bouwelementen", true, ["ArrowRight"]);

    cy.get("@bouwelementen").then((subject) => firstChildItem(subject, "afvoerbuizen"));

    shouldHaveExpandedBehaviour("afvoerbuizen", null, ["ArrowDown", "ArrowRight"]);

    cy.get("@bouwelementen").then((subject) => lastChildItem(subject, "ankers"));
  ...
  it("should have roving tabIndex to move focus with arrow keys, Home and End", () => {
    function shouldHaveFocusBehaviour(
      startElement: string,
  ...
  it("should set tabindex = 0 on clicked item", () => {
    ...
    cy.get("@complexen").should("have.attr", "tabIndex", 0);

    cy.get("@complexen")
      .then((subject) => previousSiblingItem(subject, "bouwwerken"))
      .click();

    cy.get("@bouwwerken").should("have.attr", "tabIndex", 0);
  });

  it("should emit click events", () => {
    const clickEvents = [];
    cy.get("dso-tree-view").then((treeView) => {
      treeView.get(0).addEventListener("dsoClickItem", (event: CustomEvent) => {
  ...
  it("should set active class on clicked item", () => {
    ...
    cy.get("@complexen").should("have.class", "active");

    cy.get("@complexen")
      .then((subject) => previousSiblingItem(subject, "bouwwerken"))
      .click();

    cy.get("@bouwwerken").should("have.class", "active").should("have.attr", "aria-current", "true");
  });
