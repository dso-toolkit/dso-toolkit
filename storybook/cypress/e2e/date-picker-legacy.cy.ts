describe("Date Picker (Legacy)", () => {
  ...
  it("ESCAPE should close date picker and focus toggle button", () => {
    ...
    cy.get("@date-picker-legacy").find(".dso-date__dialog").should("not.have.class", "is-active").and("not.be.visible");
  ...
  it("should only allow date characters input", () => {
    ...
    cy.get("dso-date-picker-legacy input.dso-date__input")
      .as("input")
      ...
      .invoke("getCall", 0)
      .its("args.0.detail")
      .should("deep.equal", { component: "dso-date-picker-legacy", value: "", valueAsDate: undefined })
      .get("@input")
      .type(allowedChars)
  ...
  it("keep cursor in correct position when editing date", () => {
    cy.get("dso-date-picker-legacy.hydrated")
      ...
      .realPress("{backspace}")
      .get("dso-date-picker-legacy input.dso-date__input")
