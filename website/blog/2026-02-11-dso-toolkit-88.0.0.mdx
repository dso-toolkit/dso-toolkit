...
- **Breaking**: De active prop is nu volledig stateless en wordt niet langer intern aangepast door het component. De
  verantwoordelijkheid voor het beheren van de active state ligt nu volledig bij de afnemer.
- Bij een klik emit de `Info Button` het `dsoToggle` event, zolang het `slot name="toggletip"` **niet** is gevuld.
- Het `slot name="toggletip"` is toegevoegd om de `Toggletip` functionaliteit door `Info Button` te laten
  verzorgen. Wanneer dit slot is gevuld, neemt de `Info Button` het tonen en verbergen van de `Toggletip` over op
  basis van klik op de `Info Button`. Dit is nieuwe optionele functionaliteit; bestaande implementaties zonder dit
  slot blijven werken zoals voorheen.
