...
- We voegen de suffix `(Deprecated)` toe aan de naam van het component:
  - Dit doen we in de property `title` van de Storybook metadata van het component voor de HTML/CSS implementatie, bijv. in `whitebox.css-stories.ts` en/of voor de Core implementatie, bijv. in `tooltip.core-stories.ts`. Etaleren we het component ook in Storybook, dan voegen we ook daar de suffix `(Deprecated)` toe.
