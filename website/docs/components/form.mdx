...
Door het toggelen van de `open` property kan men de inhoud tonen of verbergen. De `dso-expandable` maakt gebruik van een animatie wanneer je de prop `enable-animation` op `true` zet.
