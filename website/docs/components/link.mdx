...
Er zijn verschillende anchor-elementen:
...
<StorybookComponent implementations={["html-css"]} variant="download-link" />

#### External Link

<StorybookComponent implementations={["html-css"]} variant="external-link" />

#### Link With Icon

<StorybookComponent implementations={["html-css"]} variant="link-with-icon" />

#### Mail Link With Icon

<StorybookComponent implementations={["html-css"]} variant="mail-link-with-icon" />

#### Phone Link With Icon

<StorybookComponent implementations={["html-css"]} variant="phone-link-with-icon" />
...
- Gebruik een label voor duidelijkheid en toegankelijkheid. Screenreader-software voegt vaak audio-feedback toe wanneer het een link leest. Schermvergrotende software doet dat niet. Een label gebruiken lost dat op.
- Ga na of de tekst van de link duidelijk is voor iemand die het bijbehorende icoontje niet ziet. Geef eventueel een tekstueel alternatief.
...
- Als de link verwijst naar een deel van de huidige pagina ('same page link'), dan moet je dit voor gebruikers van screenreaders duidelijk maken door "spring naar" `sr-only` voor de linktekst te zetten. Bijvoorbeeld: `<span class="sr-only">spring naar</span> inhoudsopgave`.
