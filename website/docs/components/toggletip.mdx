...
# Toggletip (Deprecated)

<StorybookComponent implementations={["core", "angular", "react"]} name="toggletip-deprecated" variant="toggletip" />

Toggletips geven aanvullende informatie in de gebruikersinterface. Ze lijken erg op tooltips, behalve dat de tip niet wordt weergegeven bij hover of focus. Een toggletip is gekoppeld aan een trigger (info button) en maakt gebruik van het Rich Content component om tekst te structureren en interactieve elementen te plaatsen.
