...
# Check for uncommitted changes after build (particularly README.md files generated by Stencil)
RUN if [ -n "$(git status --porcelain)" ]; then \
      echo "❌ Build process has modified files that should be committed to git:"; \
      git status --porcelain; \
      echo ""; \
      echo "💡 Please commit these changes to ensure the build pipeline and git repository stay in sync."; \
      exit 1; \
    fi
